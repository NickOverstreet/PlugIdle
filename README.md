...
- **🏆 Goals**: 61 achievements that unlock as you hit milestones — each one permanently adds **+1% production** (the Grid Bonus), so chasing goals makes you stronger.
- **Offline earnings**: your cords keep working while you're away (10% efficiency, or 50% with the Night Shift core upgrade; capped at 24h), shown in a "Welcome back" summary.
...
- **Installable PWA**: `manifest.webmanifest` + a service worker (`sw.js`) that caches assets for offline play (network-first for navigations, cache-first for assets). The GitHub Pages deploy workflow auto-rewrites the `CACHE` version in `sw.js` to the commit SHA on every deploy, so returning PWA players always get fresh assets without a manual bump.
...
- Content: **27 cord tiers · 46 upgrades · 23 core (prestige) upgrades · 12 challenges (6 Grid + 6 Voltlands) · 61 achievements · a hidden second world with 10 weapons + 12 zap upgrades**.
