# Play Store Listing — PlugIdle

Copy-paste source for the Play Console listing. Character limits noted per field.
...
```
Plug in cords, earn watts, prestige! A chill retro idle clicker. No forced ads.
```
...
🚫 NO DARK PATTERNS
• No forced ads — ever. Watch a rewarded ad ONLY if you want a bonus.
...
| Asset | Spec | Source |
|---|---|---|
| App icon | 512×512 PNG, ≤1 MB | `icons/icon-maskable-512.png` (full-bleed) |
| Feature graphic | 1024×500 PNG/JPG | `store/feature-graphic.png` (regenerate: `python3 scripts/make_store_assets.py`) |
| Phone screenshots | 2–8, each side 320–3840 px, 16:9–9:16 | take on device/emulator — see shot list below |

### Screenshot shot list (portrait, take on a phone or emulator)

1. **Plug tab** mid-game: socket + a healthy list of cord generators with affordable buys lit up.
2. **A power surge on screen** (the spark) or an active ×7 Frenzy buff bar — shows the action moment.
...
6. (after monetization ships) **Store section** showing rewarded bonuses — reinforces "optional ads only".

Tip: play a save into the mid-game first (or import one) so numbers look alive;
screenshots of a fresh save read as empty.
